...
import { superAdminRemoveUserFromTeam, superAdminChangeUserRole } from '@/app/super-admin-actions'
...
export function SuperAdminTeamMemberActions({
  ...
  const handleChangeRole = async () => {
    ...
    try {
      ...
      if (result && typeof result === 'object' && 'error' in result) {
        console.error('Change role error:', result.error)
        alert('Error: ' + result.error)
      } else {
      ...
      <AlertDialog open={showRemoveDialog} onOpenChange={setShowRemoveDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to remove {userEmail} from this team? 
      ...
      <AlertDialog open={showRoleChangeDialog} onOpenChange={setShowRoleChangeDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to change {userEmail}&apos;s role from {currentRole} to {newRole}?
              {newRole === 'admin' && (
                <span className="mt-2 block text-sm">
                  <strong>Admin privileges include:</strong> Managing team members, deleting team, and full access to team resources.
                </span>
              )}
              {newRole === 'member' && (
                <span className="mt-2 block text-sm">
                  <strong>Note:</strong> They will lose admin privileges and cannot manage other team members.
                </span>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
