...
export interface PromptProps
  ...
  messages?: any[] // Add messages to detect if conversation has started
}
  ...
  const readFileAsBase64 = (file: File): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        ...
        // Keep the full data URL with prefix for vision models
        resolve(result)
      }
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      try {
        ...
        const attachment: FileAttachment = { name: file.name, type: file.type, content, size: file.size }
      ...
      <div className="premium-input relative flex max-h-60 w-full grow flex-col overflow-hidden transition-all duration-200">
        <div className="absolute left-3 top-1/2 z-10 flex -translate-y-1/2 items-center space-x-2">
          <Tooltip>
            ...
            <TooltipContent side="top" className="border border-border/50 bg-popover/95 backdrop-blur-sm">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <SelectTrigger className={cn(
                  ...
                  <SelectValue>
                    {teams.find(t => t.id === selectedTeamId)?.name?.substring(0, 13) || 'Team'}
                  </SelectValue>
                </SelectTrigger>
              </TooltipTrigger>
              <TooltipContent side="top" className="border border-border/50 bg-popover/95 backdrop-blur-sm">
          ...
          {/* Model select: move TooltipTrigger to wrap SelectTrigger (not the whole Select) to avoid ref warning */}
          <Select
            value={selectedModel}
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <SelectTrigger className={cn(
                  ...
                  <SelectValue>
                    {AVAILABLE_MODELS.find(m => m.id === selectedModel)?.name?.split(' ')[0] || 'Model'}
                  </SelectValue>
                </SelectTrigger>
              </TooltipTrigger>
              <TooltipContent side="top" className="border border-border/50 bg-popover/95 backdrop-blur-sm">
        ...
        <div className="absolute right-3 top-1/2 z-10 -translate-y-1/2">
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                <IconArrowElbow className={cn("h-4 w-4 transition-transform duration-200", input.trim() && "rotate-0")} />
